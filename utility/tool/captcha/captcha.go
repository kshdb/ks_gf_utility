...
func (captcha *Captcha) getFormulaMixData() (string, []string) {
	...
	opArr := []rune(operator)
	opRand := opArr[captcha.RangeRand(0, 1)] //随机算法
...
func (captcha *Captcha) initCanvas() *image.RGBA {
	...
	for x := 0; x < captcha.W; x++ {
		for y := 0; y < captcha.H; y++ {
			dest.Set(x, y, color.RGBA{r, g, b, 106}) //设定alpha图片的透明度
		}
	}
...
// 增加干扰圆圈
func (captcha *Captcha) doRound(gc *draw2dimg.GraphicContext) {
	// 设置干扰圆圈
	for n := 0; n < 4; n++ {
		// 设置圆的颜色
		gc.SetStrokeColor(color.RGBA{3, 71, 197, 255})
		gc.SetLineWidth(1.5)
		radius1 := gconv.Float64(grand.N(6, 20))
		radius2 := gconv.Float64(grand.N(1, 2))
		gc.BeginPath()
		gc.ArcTo(float64(captcha.RangeRand(4, int64(captcha.W)+10)), float64(captcha.RangeRand(6, int64(captcha.H)+5)), radius1, radius1/radius2, gconv.Float64(n), 2*math.Pi)
		gc.Stroke()
	}
}
